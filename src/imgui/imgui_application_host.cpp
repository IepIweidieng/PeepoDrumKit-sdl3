...
namespace ApplicationHost
{
	...
	static void EmbeddedIcons_MakeGlyphBitmapSubpixel(const ImFontBaked* baked, u32* output, i32 out_w, i32 out_h, i32 out_stride, f32 scale_x, f32 scale_y, f32 shift_x, f32 shift_y, i32 glyph)
	{
		...
		// const i32 resizeResult = stbir_resize_uint8_srgb(
		// 	reinterpret_cast<const u8*>(&EmbeddedIconsPixelData[icon.OffsetIntoPixelData]), icon.Size.x, icon.Size.y, (icon.Size.x * sizeof(u32)),
		// 	reinterpret_cast<u8*>(output), out_w, out_h, out_stride, ::stbir_pixel_layout::STBIR_RGBA);
	}
...
#undef X_KERNEL
	...
	static bool ImplEmbeddedIcons_FontBakedLoadGlyph(ImFontAtlas* atlas, ImFontConfig* src, ImFontBaked* baked, void* loader_data_for_baked_src, ImWchar codepoint, ImFontGlyph* out_glyph)
	{
	...
	i32 EnterProgramLoop(const StartupParam& startupParam, UserCallbacks userCallbacks)
	{
		...
		while (!done)
		{
			...
			GlobalState.IsAnyWindowFocusedThisFrame = GlobalIsWindowFocused;
